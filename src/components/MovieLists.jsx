...
function MovieLists() {
  ...
  useEffect(() => {
    ...
    if (localData) {
      setMovieData(JSON.parse(localData));
      setLoading(false);
    } else {
      setLoading(true);
      ...
      getMovies(currentPage)
        .then((movieData) => {
          ...
          localStorage.setItem(currentPage, JSON.stringify(movieData));
          setLoading(false);
        })
        .catch((err) => {
          setError(err.message ?? "An error happend, Refresh the page");
          setLoading(false);
        });
    }
  }, [currentPage]);
